...
const useNewInboxGeneral = ({ tenantId, ModuleCode, filters, middleware = [], config = {} }) => {
  ...
  let { workflowFilters, searchFilters, limit, offset, sortBy, sortOrder, incidentType, phcType, applicationNumber } = fetchFilters(filters);

  const query = useQuery(
    ["INBOX", workflowFilters, searchFilters, ModuleCode, limit, offset, sortBy, sortOrder, incidentType, phcType, applicationNumber],
    () =>
      InboxGeneral.Search({
        inbox: { tenantId, processSearchCriteria: workflowFilters, moduleSearchCriteria: { ...searchFilters, sortBy, sortOrder, incidentType, phcType, applicationNumber }, limit, offset },
      }),
    {
      select: (data) => {
